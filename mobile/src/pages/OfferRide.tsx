...
import { ArrowLeft, MapPin, Car, ChevronDown, Map } from 'lucide-react'
...
function LocationInput({ label, placeholder, color, onSelect, onMapOpen }: {
  label: string; placeholder: string; color: string
  onSelect: (r: LocationResult) => void; onMapOpen: () => void
}) {
  ...
  const handleChange = (v: string) => {
    setQuery(v); setSelected('')
    if (timer) clearTimeout(timer)
    setTimer(setTimeout(async () => {
      if (v.length >= 3 && v !== selected) { const r = await searchLocation(v); setResults(r); setOpen(true) }
    }, 400))
  }
  ...
  return (
    <div className="relative">
      ...
      <div className="flex items-center gap-2 bg-[#f5f5f5] rounded-2xl px-4 py-3.5">
        ...
        />
        <button onClick={onMapOpen} className="w-8 h-8 rounded-xl bg-white flex items-center justify-center shadow-sm shrink-0">
          <Map size={14} className="text-[#714B67]" />
        </button>
      </div>
          ...
          {results.map((r, i) => (
            <button key={i} className="w-full px-4 py-3 text-left text-sm border-b border-[#f5f5f5] last:border-0 active:bg-[#f5f5f5]"
              onClick={() => {
                ...
                setQuery(lbl); setSelected(lbl); onSelect(r); setOpen(false)
              }}>
  ...
  const submit = async () => {
    ...
    try {
      await api.post('/rides', {
        ...
        pickupLat: parseFloat(pickup.lat), pickupLng: parseFloat(pickup.lon),
        ...
        availableSeats: parseInt(seats), farePerSeat: parseFloat(fare),
      })
    ...
    <div className="h-full flex flex-col bg-[#f5f5f5]">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 safe-bottom">
        ...
        <div className="bg-white rounded-3xl p-4 space-y-3">
          ...
          <LocationInput label="From" placeholder="Howrah Station, Kolkata" color="#16a34a"
            onSelect={setPickup} onMapOpen={() => setMapFor('pickup')} />
          <LocationInput label="To" placeholder="Sector V, Salt Lake, Kolkata" color="#dc2626"
            onSelect={setDest} onMapOpen={() => setMapFor('dest')} />
        </div>
          ...
          {vehicles.length === 0 ? (
            ...
            <div className="relative">
              <select value={vehicleId} onChange={e => setVehicleId(e.target.value)}
                ...
                {vehicles.map(v => <option key={v.id} value={v.id}>{v.model} · {v.registration}</option>)}
              </select>
